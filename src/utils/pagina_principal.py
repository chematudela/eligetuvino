...
def pagina_principal():
    def cargar_frases():
        try:
            with open("utils/frases.txt", "r", encoding="utf-8") as file:
    ...
    <div style="text-align: center; background-color: #800000; padding: 20px; border-radius: 15px; margin: 20px;">
        ...
        <p class="frase" style="cursor: pointer;" onclick="document.getElementById('cambiar_frase').click();">
    ...
    # Botón oculto para cambiar la frase
    if st.button("Cambiar frase", key='cambiar_frase'):
        cambiar_frase()
